...
class CorrectDataValidator:
    ...
    def check_age_range(self):
        ...
        months = sorted(set(item['month'] for item in self.data))
        print(f"年龄范围: {min(months)} - {max(months)} 月龄")
        print(f"包含的月龄: {months}")
        
        # 检查是否有缺失的月龄
        expected_months = [6, 7, 8, 9, 10, 11, 12, 15, 18, 21, 24, 27, 30, 33, 36, 42, 48, 54, 60, 66, 72, 78, 84]
        missing_months = [m for m in expected_months if m not in months]
        if missing_months:
            print(f"缺失的月龄: {missing_months}")
        else:
            print("✅ 所有预期月龄都存在")
...
def main():
    """主函数"""
    # 验证正确解析的数据
    data_file = "data/correct_scale_data.json"
    
    if not os.path.exists(data_file):
        print(f"❌ 数据文件不存在: {data_file}")
        return
    
    validator = CorrectDataValidator(data_file)
